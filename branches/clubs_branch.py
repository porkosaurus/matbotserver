...
def get_all_clubs():
    """Fetch all club names from the organizations table"""
...
def get_club_recommendations(question, all_clubs):
    ...
    response = anthropic.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""You are tasked with recommending clubs based on a student's interests. Here is the complete list of available clubs:
...
def get_club_details(club_names):
    ...
    for name in club_names:
        cursor.execute("SELECT * FROM organizations WHERE Name = %s", (name,))
...
def create_advisor_response(question, club_details):
    ...
    response = anthropic.messages.create(
        ...
        messages=[{
            "role": "user",
            "content": f"""You are a friendly student advisor helping a student find clubs that match their interests. The student asked: "{question}"
...
def answer_clubs_question(question):
